...
void	display_text(bonus_t *bonus, corewar_t *core)
{
	...
	sfText_setString(bonus->val_carac[0], str);
	free(str);
	str = transform_integer_in_str(core->nbr_live_total);
	sfText_setString(bonus->val_carac[1], str);
	free(str);
	str = transform_integer_in_str(core->cycle_to_die);
	sfText_setString(bonus->val_carac[2], str);
	free(str);
	str = transform_integer_in_str(core->nb_champion);
	sfText_setString(bonus->val_carac[3], str);
	free(str);
	for (unsigned int i = 0; bonus->name_champ[i] != NULL; i++)
		sfRenderWindow_drawText(bonus->win, bonus->name_champ[i], NULL);
	for (unsigned int i = 0; bonus->caract[i] != NULL; i++) {
		sfRenderWindow_drawText(bonus->win, bonus->caract[i], NULL);
		sfRenderWindow_drawText(bonus->win, bonus->val_carac[i], NULL);
	}
}
void	display(bonus_t *bonus, corewar_t *core)
{
	...
	sfTexture_updateFromPixels(bonus->tex_mem, frame->pixels, 1200, 800,
									0, 0);
